...
public class pythonInspArraySum : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ray[2] = valor3;
        ray[3] = valor4;
        Debug.Log(sumArray(ray, 0, 0));
    }

    // Update is called once per frame
    private int sumArray(int[] n,int counter,int prevVal)
    {
        if (counter < n.Length - 1)
        {
            prevVal += n[counter];
            Debug.Log("entro en recursividad");
            return sumArray(n, counter + 1, prevVal);
        }
        else
        {
            prevVal += n[counter];
